...
- **复习规划**：输入考试日期 + 每天可用时长 → 生成按科目/优先级分层的周计划（纯本地算法，可离线，含每周具体日期与里程碑）
- **思维导图**：把考点分析结果导出为 Markdown / Mermaid 思维导图（含高频关键词子节点，可直接在支持 Mermaid 的工具中渲染）
- **错题本**：结构化录入 → 科目/标签管理 → AI 自动复盘（错因 + 知识点回归 + 同类题预测）→ 反复错题统计
- **AI 阅读出题**：上传你自己的阅读材料 → AI 按考研风格命制四选一选择题（细节/主旨/推理/词义/态度），答案解析可折叠
- **错题复盘**：粘贴错题 → AI 生成"错因分析 + 知识点回归 + 同类题预测"
- **Gradio 界面**：浏览器打开即用，无需命令行（含考点分析/复习规划/错题复盘/缓存管理/错题本/AI 出题六个页签）
